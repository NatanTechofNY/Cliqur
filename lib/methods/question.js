Meteor.methods({
	...
	toggleQuestion: function(data) {
		...
		Questions.update({"parentSessionId": data.sessionId, "_id": {"$ne": data.questionId}}, {"$set": {"isPublic": false}}, {multi: true});
		Questions.update({"parentSessionId": data.sessionId, "_id": data.questionId}, {"$set": {"isPublic": true}});
	},
