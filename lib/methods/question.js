Meteor.methods({
	...
	unsetAllQuestions: function(data) {
		...
		Questions.update({"parentSessionId": data.sessionId}, {"$set": {"isPublic": false}}, {multi: true});
	},
	toggleQuestion: function(data) {
		...
		Questions.update({"_id": data.questionId}, {"$set": {"isPublic": true}});
	},
