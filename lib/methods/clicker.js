Meteor.methods({
	resetClickerData: function(data) {
		if(typeof data.sessionId !== "string" || !Sessions.findOne({"_id": data.sessionId}))
			throw new Meteor.Error('Invalid session identifier');

		Sessions.update({"_id": data.sessionId}, {$set: {clickerData: {updatedAt: new Date(), responses: []}}});
	...
	addResponses: function(data){
		...
		var sess = Sessions.findOne({"_id": data.sessionId});
		...
		Sessions.update({"_id": data.sessionId}, {$set: {clickerData: {updatedAt: new Date(), responses: resps}}});
	},
